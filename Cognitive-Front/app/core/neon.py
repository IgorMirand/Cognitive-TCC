...
class Database:
    ...
    def change_password(self, user_id, old_pass, new_pass):
        ...
        try:
            ...
            # Pega o erro da Cognitive-Front-API (ex: "Senha atual incorreta")
    ...
    def get_patient_count(self, psicologo_id):
        try:
            ...
            # --- DEBUG ---
            print(f"Erro Cognitive-Front-API Stats: {res.status_code} - {res.text}")
            return False, 0
        except Exception as e: 
    ...
    def get_pacientes_do_psicologo(self, psicologo_id):
        try:
            ...
            if res.status_code == 200:
                # Cognitive-Front-API retorna lista de listas [[1, "joao"]], convertemos para lista de tuplas
    ...
    def get_psicologo_id_by_paciente(self, paciente_id):
        """
        Busca qual é o ID do psicólogo vinculado a este paciente via Cognitive-Front-API.
        """
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] get_psicologo_id_by_paciente: {e}")
            return None
        

    def validar_codigo_master(self, codigo):
        """
        Verifica na Cognitive-Front-API se o código mestre é válido.
        ...
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] validar_codigo_master: {e}")
            return None

    def marcar_codigo_master_usado(self, codigo_id, user_id):
        """
        Avisa a Cognitive-Front-API para marcar esse código como utilizado por este usuário.
        """
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] marcar_codigo_master_usado: {e}")
            return False
        
    def gerar_codigo_paciente(self, psicologo_id):
        """
        Solicita à Cognitive-Front-API a geração de um novo código de vínculo para paciente.
        Retorna a string do código (ex: "A4B-1C2") ou None se der erro.
        """
        try:
            # A rota na Cognitive-Front-API é: POST /codigos/gerar/{psicologo_id}
            ...
            print(f"[Cognitive-Front-API ERRO] gerar_codigo: {res.status_code} - {res.text}")
            return None
    ...
    def get_pacientes_do_psicologo_com_nomes(self, psicologo_id):
        """
        Retorna uma lista de tuplas (id_paciente, nome_paciente) via Cognitive-Front-API.
        Usado no menu dropdown para selecionar pacientes.
        """
        try:
            # Reutiliza a rota que já criamos na Cognitive-Front-API
            url = f"{self.base_url}/psicologo/{psicologo_id}/pacientes"
            res = requests.get(url)
            
            if res.status_code == 200:
                data = res.json()
                # A Cognitive-Front-API retorna lista de listas: [[1, "Joao"], [2, "Maria"]]
            ...
            print(f"[ERRO Cognitive-Front-API] get_pacientes_do_psicologo_com_nomes: {e}")
            return []
    
    def get_agenda_psicologo(self, psicologo_id):
        """
        Busca a agenda do psicólogo via Cognitive-Front-API.
        Retorna uma lista de tuplas: [(id, data_hora, paciente_id), ...]
        """
        try:
            # Chama a rota GET /agenda/psicologo/{id} que criamos na Cognitive-Front-API
            url = f"{self.base_url}/agenda/psicologo/{psicologo_id}"
            res = requests.get(url)
            
            if res.status_code == 200:
                data = res.json()
                # A Cognitive-Front-API retorna {'agenda': [[1, '2023-10...', 5], ...]}
            ...
            print(f"[ERRO Cognitive-Front-API] get_agenda_psicologo: {e}")
            return []
        
    def get_user_details(self, user_id):
        """
        Busca detalhes do usuário (nome, email, nascimento) via Cognitive-Front-API.
        """
        try:
            ...
            if res.status_code == 200:
                ...
                if data.get('data_nascimento'):
                    try:
                        ...
                        # A Cognitive-Front-API costuma devolver YYYY-MM-DD (ISO)
            ...
            print(f"[ERRO Cognitive-Front-API] get_user_details: {e}")
            return None
    
    def vincular_paciente_por_codigo(self, paciente_id, codigo):
        """
        Envia o código para a Cognitive-Front-API tentar vincular este paciente a um psicólogo.
        Retorna: (True, "Sucesso") ou (False, "Mensagem de Erro")
        """
        try:
            url = f"{self.base_url}/vincular"
            
            # A rota na Cognitive-Front-API espera um JSON com 'paciente_id' e 'codigo'
            payload = {
            ...
            # Se deu erro (código inválido, já vinculado, etc), pega a mensagem da Cognitive-Front-API
            erro_msg = res.json().get("detail", "Erro desconhecido ao vincular.")
            return False, erro_msg

        except Exception as e:
            print(f"[ERRO Cognitive-Front-API] vincular_paciente_por_codigo: {e}")
            return False, "Falha de conexão com o servidor."
        
    def update_user_details(self, user_id, username, email, data_nascimento):
        """
        Envia os dados atualizados para a Cognitive-Front-API.
        ...
        try:
            ...
            # Se der erro (ex: email já existe), pega a mensagem da Cognitive-Front-API
            erro_msg = res.json().get("detail", "Erro ao atualizar.")
            return False, erro_msg

        except Exception as e:
            print(f"[ERRO Cognitive-Front-API] update_user_details: {e}")
            return False, "Erro de conexão."
    
    def adicionar_disponibilidade(self, psicologo_id, data_hora_obj):
        """
        Envia um novo horário de disponibilidade para a Cognitive-Front-API.
        data_hora_obj: objeto datetime (ex: datetime(2023, 10, 25, 14, 30))
        """
        try:
            # A Cognitive-Front-API espera o formato de string ISO 8601
            ...
            # Pega mensagem de erro da Cognitive-Front-API se houver
            erro_msg = res.json().get("detail", "Erro ao salvar horário.")
            return False, erro_msg

        except Exception as e:
            print(f"[ERRO Cognitive-Front-API] adicionar_disponibilidade: {e}")
            return False, "Erro de conexão com o servidor."
        
    def excluir_horario(self, agenda_id):
        """
        Remove um horário da agenda via Cognitive-Front-API.
        """
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] excluir_horario: {e}")
            return False, "Erro de conexão."
    ...
    def get_horarios_paciente(self, psicologo_id, meu_id):
        ...
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] get_horarios_paciente: {e}")
            return []
    ...
    def reservar_horario(self, agenda_id, paciente_id):
        ...
        try:
            ...
            # Retorna o erro da Cognitive-Front-API (ex: "Horário já reservado")
    ...
    def agendar_consulta(self, agenda_id, paciente_id):
        ...
        """
        try:
            # A rota na Cognitive-Front-API é: PUT /agenda/{id}/reservar
            url = f"{self.base_url}/agenda/{agenda_id}/reservar"
            ...
            # Pega a mensagem de erro da Cognitive-Front-API (ex: "Horário já ocupado")
            msg = res.json().get("detail", "Erro ao agendar.")
            return False, msg
            
        except Exception as e:
            print(f"[ERRO Cognitive-Front-API] agendar_consulta: {e}")
            return False, "Erro de conexão."
    ...
    def get_anotacoes_paciente(self, psicologo_id, paciente_id):
        ...
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] get_anotacoes_paciente: {e}")
            return []
    ...
    def salvar_anotacao_psicologo(self, psicologo_id, paciente_id, texto, data_hora_iso):
        ...
        """
        try:
            # Rota da Cognitive-Front-API que criamos anteriormente (POST /consultas)
            ...
            # Tenta pegar a mensagem de erro detalhada da Cognitive-Front-API
            erro_msg = res.json().get("detail", "Erro ao salvar relatório.")
            return False, erro_msg

        except Exception as e:
            print(f"[ERRO Cognitive-Front-API] salvar_anotacao_psicologo: {e}")
            return False, "Erro de conexão com o servidor."

    def enviar_convite(self, psicologo_id, email_paciente):
            """
            Pede para a Cognitive-Front-API gerar um código e enviar por e-mail (Resend).
            """
            try:
                ...
                # Envia a requisição para a Cognitive-Front-API
                res = requests.post(url, json=payload)
                
                if res.status_code == 200:
                    return True, "Convite enviado com sucesso!"
                
                # Pega a mensagem de erro detalhada da Cognitive-Front-API
                erro_msg = res.json().get("detail", "Erro ao enviar convite.")
                return False, erro_msg

            except Exception as e:
                print(f"[ERRO Cognitive-Front-API] enviar_convite: {e}")
                return False, "Erro de conexão com o servidor."
...
# --- Adicione dentro da class Database ---
    ...
    def get_powerbi_url(self):
        """
        Busca o link atualizado do Power BI na Cognitive-Front-API.
        """
        try:
            ...
            print(f"[ERRO Cognitive-Front-API] get_powerbi_url: {e}")
            return None